...
class BPConfig() :
    ...
    @classmethod
    def default_config_uri(cls, workspace) :
        return f"s3://{workspace}/automl-blueprint/config/blueprint-config.json"

    @classmethod
    def default_config_prefix(cls, workspace) :
        return f"s3://{workspace}/automl-blueprint/config"
    ...
    def _persist_bp_config(cls, config_uri = None) :
        ...
        if not config_uri :
            config_uri = cls.default_config_prefix(cls.workspace)
